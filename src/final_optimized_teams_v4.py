#!/usr/bin/env python3
"""
Final version with improved GK pairing strategy:
- Bench GK should be from same team as starting GK when possible
- Allow lower score threshold for same-team backup GKs
...
def build_optimal_teams(predictions_file, num_teams=200):
    ...
    for team, gks in sorted(gk_by_team.items()):
        if len(gks) > 1:
            gk_list = ', '.join([f'{gk["name"]} (£{gk["price"]}m, score: {gk["score"]:.2f})' for gk in gks])
...
def main():
    ...
    output_file = "../data/cached_merged_2024_2025_v2/top_200_teams_final_v6.csv"
    
    print("Creating optimized teams with improved GK pairing strategy...")
