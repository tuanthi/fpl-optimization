#!/usr/bin/env python3
"""
Final version with standardized output format:
- Column order: captain, formation, budget, gw1_score, 5gw_estimated
- Then: GK1-GK2, DEF1-DEF5, MID1-MID5, FWD1-FWD3 (with all associated columns)
- GK pairing strategy included
"""
...
def is_valid_player(player_name, player_club, player_score, role, minutes_dict=None, is_backup_gk=False):
    ...
    if role == 'GK':
        # Special case: Allow lower score for backup GKs from same team
        if is_backup_gk:
            # Backup GKs can have lower scores (0.5-6.0)
            if player_score < 0.5 or player_score > 6.0:
                return False
        else:
            # Starting GKs should have higher scores (2.0-6.0)
...
def find_backup_gk(starting_gk, all_gks, team_counts, max_price=4.5, minutes_dict=None):
    ...
    same_team_backups = [
        ...
        # Use special validation for backup GKs from same team
        and is_valid_player(gk['name'], gk['club'], gk['score'], 'GK', minutes_dict, is_backup_gk=True)
    ]
    ...
    other_backups = [
        ...
        # Use normal validation for other team GKs
        and is_valid_player(gk['name'], gk['club'], gk['score'], 'GK', minutes_dict, is_backup_gk=False)
    ]
...
def build_optimal_teams(predictions_file, num_teams=200):
    """Build teams with improved filtering and standardized output format"""
    ...
    for _, row in df.iterrows():
        ...
        # Validate player (not as backup GK yet)
        if is_valid_player(player_name, row['club'], row['weighted_score'], row['role'], minutes_dict, is_backup_gk=False):
...
def main():
    ...
    output_file = "../data/cached_merged_2024_2025_v2/top_200_teams_final_v7.csv"
    
    print("Creating optimized teams with standardized output format...")
    ...
    if teams_df is not None and not teams_df.empty:
        ...
        # Show sample of column order
        print("\nColumn order (first 20 columns):")
        print(list(teams_df.columns[:20]))
    else:
