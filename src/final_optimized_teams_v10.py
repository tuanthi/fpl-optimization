#!/usr/bin/env python3
"""
Fixed version that ensures top players like Salah are included:
- Always includes highest scoring players when budget allows
...
def build_optimal_teams(predictions_file, num_teams=200):
    """Build teams ensuring top players are included with proper captain selection"""
            ...
            for include_second_mid in [True, False]:
                ...
                for def_start in range(0, min(10, len(positions['DEF']) - formation['DEF'])):
                    ...
                    for pos_name, needed in formation.items():
                        ...
                        already_added = sum(1 for p in team['players'] if p['role'] == pos_name)
                        needed -= already_added
...
if __name__ == "__main__":
    ...
    # Save results
    output_file = "/Users/huetuanthi/dev/dokeai/fpl/data/cached_merged_2024_2025_v2/top_200_teams_final_v11.csv"
    ...
    # Verify Salah is in teams
    salah_count = 0
    for idx, team in teams_df.iterrows():
        for pos in ['MID']:
            for i in range(1, 6):
                if f'{pos}{i}' in team and 'Salah' in str(team.get(f'{pos}{i}', '')):
                    salah_count += 1
                    break
    print(f"\nMohamed Salah appears in {salah_count}/{len(teams_df)} teams")