#!/usr/bin/env python3
"""
Fixed version with proper 3 players per team constraint:
- Correctly counts all players including bench when checking team limits
- Ensures no team has more than 3 players total (including GKs)
"""
...
def build_optimal_teams(predictions_file, num_teams=200):
    """Build teams with fixed GK hierarchy and proper 3 player per team constraint"""
    ...
    if len(skipped_players) < 20:  # Only show if reasonable number
        for player, club, role, score, reason in skipped_players[:10]:
            print(f"  - {player} ({club}, {role}): score={score:.2f}, reason={reason}")
    ...
    # Try different captain choices (top players from MID/FWD)
    captain_candidates = (
        sorted(positions['MID'][:5] + positions['FWD'][:3], key=lambda x: x['score'], reverse=True)
    )
    
    print(f"\nGenerating teams with {len(captain_candidates)} captain candidates...")
    
    for formation in formations:
        for captain in captain_candidates:
            captain_pos = captain['role']
            
            for gk_idx, starting_gk in enumerate(main_gks_only[:10]):  # Top 10 main GKs
                # Initialize team
                team = {
                    'players': [],
                    'bench': [],
                ...
                for pos_name, needed in formation.items():
                    ...
                    if pos_name == 'GK':
                        # Always add the starting GK
                        team['players'].append(starting_gk)
                        team['cost'] += starting_gk['price']
                        team_counts[starting_gk['club']] = team_counts.get(starting_gk['club'], 0) + 1
                        continue
                    elif pos_name == captain_pos and captain_pos != 'GK':
                        needed -= 1  # Captain already added
                    
                    # Get candidates for this position
                    if pos_name == 'GK':
                        candidates = main_gks_only[gk_idx:gk_idx+10]  # Use main GKs only for starting XI
                    else:
                        candidates = positions[pos_name][:30]  # Top 30 for each position
                ...
                if starting_gk:
                    ...
                    if backup_gk:
                        ...
                        # ALWAYS increment team count, even for same team
                ...
                # Calculate scores
                team['starting_score'] = calculate_team_score(team['players'], captain['name'])
                team['total_score'] = team['starting_score']  # Could add bench boost logic
                
                # Verify 3 player per team constraint
                constraint_violated = False
                for club, count in team_counts.items():
                    if count > 3:
                        print(f"WARNING: Team constraint violated! {club} has {count} players")
    ...
    print(f"\nGenerated {len(teams)} valid teams")
...
if __name__ == "__main__":
    ...
    print("Building optimal FPL teams with fixed 3 player per team constraint...")
    teams_df = build_optimal_teams(predictions_file, num_teams=200)
    
    # Save results
    output_file = "/Users/huetuanthi/dev/dokeai/fpl/data/cached_merged_2024_2025_v2/top_200_teams_final_v8.csv"
    teams_df.to_csv(output_file, index=False)
    print(f"\nSaved {len(teams_df)} teams to {output_file}")
    
    # Display top teams
    print("\nTop 10 teams:")
    print(teams_df.head(10)[['captain', 'formation', 'budget', 'gw1_score', '5gw_estimated']])
    
    # Verify constraint for top teams
    print("\n\nVerifying 3 player per team constraint for top 5 teams:")
    for idx in range(min(5, len(teams_df))):
        print(f"\nTeam {idx+1}:")
        team_row = teams_df.iloc[idx]
        ...
        for pos in ['GK', 'DEF', 'MID', 'FWD']:
            for i in range(1, 6):  # Max 5 of any position
                ...
                if player_col in team_row and pd.notna(team_row[player_col]):
                    player_name = team_row[player_col]
                    if club_col in team_row:
                        club = team_row[club_col]
        ...
        for club, count in sorted(club_counts.items(), key=lambda x: -x[1]):
            if count > 1:
                print(f"  {club}: {count} players" + (" ⚠️ VIOLATION!" if count > 3 else ""))