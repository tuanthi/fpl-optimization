#!/usr/bin/env python3
"""
Enhanced version to generate 200+ teams with proper 3 player constraint
- Expanded player pool
- More captain candidates
- Better bench selection
"""
...
def build_optimal_teams(predictions_file, num_teams=200):
    ...
    # Expand captain candidates to get more variety
    captain_candidates = []
    
    # Top MIDs as captains
    captain_candidates.extend(positions['MID'][:10])
    
    # Top FWDs as captains  
    captain_candidates.extend(positions['FWD'][:8])
    
    # Sort by score to prioritize best captains
    ...
    for formation in formations:
        for cap_idx, captain in enumerate(captain_candidates):
            captain_pos = captain['role']
            
            # Try different GK options
            for gk_idx, starting_gk in enumerate(main_gks_only[:15]):  # Try more GKs
                ...
                for def_start in range(0, min(10, len(positions['DEF']) - formation['DEF'])):
                    ...
                    for pos_name, needed in formation.items():
                        ...
                        if pos_name == 'DEF':
                            ...
                            candidates = positions[pos_name][:40]  # Increased pool
                    ...
                    for role in ['DEF', 'MID', 'FWD']:
                        cheap_players = [p for p in positions[role] if p['price'] <= 5.5]  # Slightly higher for more options
...
if __name__ == "__main__":
    ...
    # Save results
    output_file = "/Users/huetuanthi/dev/dokeai/fpl/data/cached_merged_2024_2025_v2/top_200_teams_final_v9.csv"
